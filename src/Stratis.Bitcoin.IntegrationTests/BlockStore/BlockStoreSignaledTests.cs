    ...
    public class BlockStoreSignaledTests
    {
        ...
        public void CheckBlocksAnnounced_AndQueueEmptiesOverTime()
        {
            ...
            using (var builder = SmartContractNodeBuilder.Create(testRootFolder))
            {
                ...
                CoreNode nodeSync = builder.CreateTokenlessNode(network, 0, server, permissions: FederationPermissions).Start();
                ...
                CoreNode node1 = builder.CreateTokenlessNode(network, 1, server, permissions: FederationPermissions).Start();
