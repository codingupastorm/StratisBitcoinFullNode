...
namespace Stratis.SmartContracts.CLR
{
    ...
    public class StateFactory : IStateFactory
    {
        ...
        public IState Create(IStateRepository stateRoot, IBlock block, ulong txOutValue, uint256 transactionHash, string version, byte[] transientData)
        {
            var logHolder = new ContractLogHolder();
            var internalTransfers = new List<TransferInfo>();
            return new State(this.smartContractStateFactory, stateRoot, logHolder, internalTransfers, block, transactionHash, version, transientData);
        }
    }
}