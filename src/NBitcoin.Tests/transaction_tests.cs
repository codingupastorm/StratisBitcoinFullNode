...
namespace NBitcoin.Tests
{
    public class Transaction_tests
    {
        ...
        {
            Network network = this.networkMain;
        ...
        public void CanBuildWitTransaction()
        {
            ...
            {
                int expectedVSize = tx.GetVirtualSize(this.networkMain.Consensus.Options.WitnessScaleFactor);
        ...
        public void CanBuildWithKnownSignatures()
        {
            ...
            TransactionSignature signature = tx.SignInput(this.networkMain, k, coin);
            ...
            Assert.True(tx.Inputs.AsIndexedInputs().First().VerifyScript(this.networkMain, coin));
        }
        ...
        public void CanBuildTransaction()
        {
            ...
            {
                TransactionSignature sig = partiallySigned.SignInput(this.networkMain, keys[0], coin);
        ...
        public void CanPartiallySignTransaction()
        {
            ...
            partiallySigned.Sign(this.networkMain, privKeys[0], true);

            //the other private keys (note the "hex" result getting longer):
            partiallySigned.Sign(this.networkMain, privKeys[1], true);
            ...
            partiallySigned.Sign(this.networkMain, privKeys[2], true);
            partiallySigned.Sign(this.networkMain, privKeys[0], true);
        ...
        private void AssertCorrectlySigned(Transaction tx, Script scriptPubKey, ScriptVerify scriptVerify = ScriptVerify.Standard)
        {
            ...
            {
                Assert.True(Script.VerifyScript(this.networkMain, scriptPubKey, tx, i, null, scriptVerify));
            }
        }

        private static StandardTransactionPolicy EasyPolicy = new StandardTransactionPolicy(new BitcoinMain())
        {
        ...
        public void CanCheckSegwitSig()
        {
            ...
            ScriptError error;
            Assert.True(toCheck.Inputs.AsIndexedInputs().Skip(0).First().VerifyScript(this.networkMain, new Script("OP_HASH160 442afa4f034468652c571202da0bf277cb729def OP_EQUAL"), Money.Satoshis(100000), ScriptVerify.Mandatory, out error));
        }
        ...
        private static void CanCheckSegwitSigCore(Transaction tx, int input, Money amount, string scriptCodeHex = null)
        {
            ...
            {
                PayToWitPubkeyHashScriptSigParameters param1 = PayToWitPubKeyHashTemplate.Instance.ExtractWitScriptParameters(new BitcoinMain(), tx.Inputs[input].WitScript);
                Assert.NotNull(param1);
                WitKeyId param2 = PayToWitPubKeyHashTemplate.Instance.ExtractScriptPubKeyParameters(new BitcoinMain(), param1.PublicKey.GetSegwitAddress(new BitcoinMain()).Hash.ScriptPubKey);
            ...
            ScriptError err;
            bool r = Script.VerifyScript(new BitcoinMain(), scriptCode, tx, 0, amount, out err);
            Assert.True(r);
        }
        ...
        public void CanParseWitTransaction()
        {
            ...
            ScriptError error;
            Assert.False(tx.Inputs.AsIndexedInputs().First().VerifyScript(this.networkMain, new Script("0 b7854eb547106248b136ca2bf48d8df2f1167588"), out error));
        ...
        public void bip143Test()
        {
            ...
            uint256 h = Script.SignatureHash(this.networkMain, new Script(Encoders.Hex.DecodeData("76a9141d0f172a0ecb48aee1be1f2687d2963ae33f71a188ac")), tx, 1, SigHash.All, Money.Satoshis(0x23c34600L), HashVersion.Witness);
        ...
        public void witnessHasPushSizeLimit()
        {
            ...
            spending.Sign(this.networkMain, bob, coin);
            ScriptError error;
            Assert.True(spending.Inputs.AsIndexedInputs().First().VerifyScript(this.networkMain, coin, out error));
            spending.Inputs[0].WitScript = new WitScript(new[] { new byte[521] }.Concat(spending.Inputs[0].WitScript.Pushes).ToArray());
            Assert.False(spending.Inputs.AsIndexedInputs().First().VerifyScript(this.networkMain, coin, out error));
        ...
        public void DoNotThrowsWithSatoshiFormatAndNoOutputs()
        {
            ...
            tx.ToString(this.networkMain, RawFormat.Satoshi);
        }
        ...
        private void PlayingWithSignatures()
        {
            ...
            ECDSASignature sig1 = (PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(this.networkMain, script1).TransactionSignature.Signature);
            ECDSASignature sig2 = (PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(this.networkMain, script2).TransactionSignature.Signature);
        ...
        public void CheckScriptCoinIsCoherent()
        {
            ...
            Assert.Throws<ArgumentException>(() => ScriptCoin.Create(this.networkMain, c, key.PubKey.ScriptPubKey.WitHash.ScriptPubKey));
        }
        ...
        public void CheckWitnessSize()
        {
            ...
            tx.Outputs.Add(new TxOut(Money.Zero, new Script(OpcodeType.OP_TRUE)));
            ScriptError error;
            Assert.True(tx.Inputs.AsIndexedInputs().First().VerifyScript(this.networkMain, coin1, ScriptVerify.Standard, out error));
            ...
            tx.Inputs[0].ScriptSig = Script.Empty;
            tx.Outputs.Add(new TxOut(Money.Zero, new Script(OpcodeType.OP_TRUE)));
            Assert.False(tx.Inputs.AsIndexedInputs().First().VerifyScript(this.networkMain, coin1, ScriptVerify.Standard, out error));
                ...
                foreach (SigHash flag in new[] { SigHash.Single, SigHash.None, SigHash.All })
                {
                    foreach (bool anyoneCanPay in new[] { true, false })
                    {
                        ...
                        foreach (HashModification modification in new[] { HashModification.NoModification, HashModification.Modification, HashModification.Invalid })
                        {
                            ...
                            TransactionSignature sig = PayToPubkeyHashTemplate.Instance.ExtractScriptSigParameters(this.networkMain, signedInput.WitScript == WitScript.Empty ? signedInput.ScriptSig : signedInput.WitScript.ToScript()).TransactionSignature;
                            if (modification != HashModification.Invalid)
                            {
                                signatures.Add(sig);
        ...
        public void Tx_Valid()
        {
            ...
            foreach (TestCase test in tests)
            {
                ...
                for (int i = 0; i < tx.Inputs.Count; i++)
                {
                    ...
                    bool valid = Script.VerifyScript(this.networkMain,
        ...
        public void Tx_Invalid()
        {
            ...
            foreach (TestCase test in tests)
            {
                ...
                for (int i = 0; i < tx.Inputs.Count && fValid; i++)
                {
                    ...
                    fValid = Script.VerifyScript(this.networkMain,
                       mapprevOutScriptPubKeys[tx.Inputs[i].PrevOut],
                       tx,
                       i,
                       mapprevOutScriptPubKeysAmount.TryGet(tx.Inputs[i].PrevOut),
        ...
        public void Test_Get()
        {
            ...
            Assert.True(StandardScripts.AreInputsStandard(this.networkMain, t1, coins));
        ...
        public void CanDecodeAndEncodeRawTransaction()
        {
            ...
            foreach (TestCase test in tests)
            {
                ...
                Network network = ((string)test[1]) == "Main" ? this.networkMain : new BitcoinTest();
