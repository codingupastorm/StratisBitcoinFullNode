...
namespace Stratis.Bitcoin.Features.Miner.Tests.Controllers
{
    public class MiningControllerTest
    {
        ...
        public void Generate_With_Incorrect_Block_Count_ReturnsInvalidRequest(int? blockCount)
        {
            ...
            consensusManager.Setup(cm => cm.Tip).Returns(new ChainedHeader(this.network.Consensus.ConsensusFactory.CreateBlockHeader(), new uint256(0), this.network.Consensus.ConsensusProofOfWork.LastPOWBlock - 1));
        ...
        public void GenerateBlocksOn_PosNetwork_ConsensusTip_IsAfterLastPowBlock_ReturnsError()
        {
            ...
            Assert.Equal(string.Format(MiningController.LastPowBlockExceededMessage, this.network.Consensus.ConsensusProofOfWork.LastPOWBlock), error.Description);
        }
